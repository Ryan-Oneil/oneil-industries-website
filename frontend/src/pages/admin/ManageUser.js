...
class ManageUser extends React.Component {
        ...
        >
            <div className="ui centered grid">
                <div className="six wide column">
                    <AdminUserDetailsForm closeModal={() => this.setState({showModal: ""})}
                                          errorMessage={this.props.admin.errorMessage}
                                          message = {this.props.admin.message}
                                          userDetails = {details}
    ...
    renderQuotaModal() {
        ...
        >
            <div className="ui centered grid">
                <div className="six wide column">
                    <EditUserQuotaForm closeModal={() => this.setState({showModal: ""})}
                                       errorMessage={this.props.admin.errorMessage}
                                       message = {this.props.admin.message}
                                       ignoreQuota = {storageQuota.ignoreQuota}
                                       user = {details.username}
                                       initialValues={{'max': storageQuota.max,
                                           'ignoreQuota' : storageQuota.ignoreQuota}}
                    />
                </div>
            </div>
            ...
            <div className="ui padded grid">
                ...
                <div className="six wide column">
                    <DetailBox header="Account Details">
                        ...
                        <p><strong>Account Enabled:</strong> {details.enabled ? 'Yes' : 'No'}</p>
                        <p><strong>Role:</strong> {details.role}</p>
                        <button className="ui primary button centerButton" onClick={() => this.setState({showModal: "account"})}>Edit</button>
                    </DetailBox>
                </div>}
                ...
                <div className="six wide column">
                    <DetailBox header="Storage Quota Details">
                        <p><strong>Used Storage:</strong> {displayBytesInReadableForm(storageQuota.used)}</p>
                        <p><strong>Max Storage:</strong> {storageQuota.max} GB</p>
                        <p><strong>Ignore Quota:</strong> {storageQuota.ignoreQuota ? "Yes" : "No"}</p>
                        <button className="ui primary button centerButton" onClick={() => this.setState({showModal: "quota"})}>Edit</button>
                    </DetailBox>
                </div>}
