...
export const getTeamspeakActiveList = endpoint => dispatch => {
  apiGetCall(endpoint)
    .then(response => {
      dispatch({
        type: SERVICES_TEAMSPEAK_GET_ACTIVE_LIST_SENT,
    ...
    .catch(error => {
      if (error.response) {
        dispatch({
          type: SERVICES_TEAMSPEAK_GET_ACTIVE_LIST_FAIL,
...
export const getDiscordActiveList = endpoint => dispatch => {
  apiGetCall(endpoint)
    .then(response => {
      dispatch({
        type: SERVICES_DISCORD_GET_ACTIVE_LIST_SENT,
        payload: response.data
      });
    })
    .catch(error => {
      if (error.response) {
        dispatch({
          type: SERVICES_DISCORD_GET_ACTIVE_LIST_FAIL,
          errorMessage: error.response.data
        });
      } else {
        dispatch({
